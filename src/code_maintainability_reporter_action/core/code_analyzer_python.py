...
class CodeAnalyzerPython(CodeAnalyzer):
    def process_file(self, path: str) -> FileAnalysisResult:
        ...
        def calc_cc(path: str) -> int:
            result = subprocess.run(["rye", "run", "radon", "cc", path, "-s"], capture_output=True, text=True)
        ...
        def calc_hal(path: str) -> float:
            result = subprocess.run(["rye", "run", "radon", "hal", path], capture_output=True, text=True)
            stdout_lines: list[str] = result.stdout.split('\n')
            for line in stdout_lines:
        ...
        def calc_sloc(path:str) -> int:
            result = subprocess.run(["rye", "run", "radon", "raw", path], capture_output=True, text=True)
            stdout_lines: list[str] = result.stdout.split('\n')
            for line in stdout_lines:
